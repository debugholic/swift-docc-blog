...
목(Mocks) 사용 사례를 구축하고 테스트 취약성과의 관계를 탐구합니다.
...
테스트 더블의 주요 용도는 테스트를 용이하게 하는 것입니다. 테스트는 설정하거나 유지하기 어려울 수 있는 실제  의존성 대신 테스트 중인 시스템으로 전달됩니다.
...
* 목(Mock, Spy 포함) 
  ...
  - 이는 주로 부수 효과(side effects)를 생성하는 명령(commands)과 관련됩니다.
  - Spy는 수동으로 작성된 목에 해당합니다.

* 스텁(Stub, Dummy, Fake 포함)
  
  - SUT가 의존성 객체로부터 입력 데이터를 얻기 위해 호출하는 "들어오는 상호작용(incoming interactions)"을 에뮬레이션하는 데 도움을 줍니다. (하지만 검사하지는 않습니다.) 
  - 이는 값을 반환하고 부수 효과가 없는 쿼리(queries)와 관련됩니다. 
  - Dummy는 간단한 하드코딩된 값이고, Stub은 더 정교하며, Fake는 아직 존재하지 않는 의존성을 대체하기 위해 구현됩니다.

##### 5.1.2 목(도구) vs. 목(테스트 더블)의 구분

목이라는 용어는 모의 객체 라이브러리의 클래스 자체 (예: Moq의 Mock<T>)를 지칭할 수도 있고, 그 클래스로 생성된 특정 유형의 테스트 더블 (Mock 객체)을 의미할 수도 있습니다.

목(도구)은 목(테스트 더블)과 스텁을 모두 생성할 수 있습니다.

##### 5.1.3 스텁과의 상호작용을 검증하지 마라

스텁과의 상호작용을 검증하는 것은 흔히 발생하는 안티패턴이며, 취약한 테스트(fragile tests)로 이어집니다. 
...
GetNumberOfUsers() 호출을 검증하는 것은 리포트 생성에 필요한 데이터를 SUT가 어떻게 수집하는지에 대한 내부 구현을 검증하는 것입니다.
...
이는 SUT가 의존성 객체에 상태 변경을 유발하는 상호작용을 검사하는 목의 역할입니다.

이 예시에서 storeMock은 두 가지 역할을 하지만 서로 다른 메서드이므로, 스텁과의 상호작용을 직접 검증하지 않는다는 원칙을 위반하지 않습니다. 

즉, 데이터를 얻기 위한 상호작용(쿼리)을 검증하는 것이 아니라, 부수 효과를 발생시키는 상호작용(명령)을 검증합니다.

테스트 더블이 목과 스텁의 역할을 동시에 수행할 때, 일반적으로 "목"이라고 부르는 경향이 있습니다.

이는 목으로서의 역할(상태 변경 유발 상호작용 검사)이 스텁으로서의 역할(입력 데이터 제공)보다 더 중요하게 간주되기 때문입니다.

##### 5.1.5 목과 스텁이 커맨드 및 쿼리와 어떻게 관련되는가

커맨드 쿼리 분리 원칙(CQS 원칙)은 모든 메서드는 명령(Command)이거나 쿼리(Query)여야 하며 둘 다여서는 안 된다고 하고 있습니다.

@Image(source: 5-2.png)

* 명령(Commands)
  - 부수 효과를 발생시킵니다 (객체의 상태 변경, 파일 시스템 변경 등).
  - 값을 반환하지 않습니다 (void 반환).
  - 목은 이러한 명령을 대체하는 테스트 더블이 됩니다. 
  - 목은 SUT가 의존성 객체에 상태 변경을 유발하기 위해 호출하는 나가는 상호작용(outcoming interactions)을 에뮬레이션하고 검사하는 데 도움을 줍니다.

* 쿼리(Queries)
  - 부수 효과가 없습니다.
  - 값을 반환합니다.
  - 스텁은 이러한 쿼리를 대체하는 테스트 더블이 됩니다. 
  - 스텁은 SUT가 의존성 객체로부터 입력 데이터를 얻기 위해 호출하는 들어오는 상호작용(incoming interactions)을 에뮬레이션하는 데 도움을 줍니다 (하지만 검사하지는 않습니다).

CQS 원칙을 따르면 코드 가독성이 향상되고, 메서드의 시그니처만 보고도 그 역할(부수 효과 발생 여부 및 값 반환 여부)을 파악할 수 있습니다.

> var mock = new Mock<IEmailGateway>();
>
> mock.Verify(x => x.SendGreetingsEmail("user@email.com"));

SendGreetingsEmail() 메서드는 이메일을 전송하는 부수 효과를 발생시키는 명령입니다. 

따라서 이 메서드를 대체하는 테스트 더블은 목이 됩니다. 

테스트에서는 이메일 전송이라는 상호작용이 일어났는지 mock.Verify()로 검증합니다.
...
따라서 이 메서드를 대체하는 테스트 더블은 스텁이 됩니다. 

테스트에서는 이 메서드가 반환할 "가상의" 값을 stub.Setup().Returns()로 설정합니다.

### 5.2 관찰 가능한 행동 vs. 구현 세부 사항

테스트 취약성은 리팩토링에 대한 내성과 밀접하게 관련되어 있으며, 거짓 양성을 줄여 리팩토링 내성을 높이는 것이 중요합니다.

거짓 양성은 테스트가 코드의 구현 세부 사항에 결합되어 발생하며, 이를 피하려면 테스트가 코드의 최종 결과(end result), 즉 관찰 가능한 동작(observable behavior)을 검증해야 합니다. 

이는 테스트가 "어떻게(hows)"가 아닌 "무엇(whats)"에 집중해야 함을 의미합니다.

##### 5.2.1 관찰 가능한 동작은 public API와 동일하지 않다

모든 프로덕션 코드는 두 가지 차원을 따라 분류될 수 있습니다.

* Public API vs. private API
* 관찰 가능한 동작 vs. 구현 세부 사항

잘 설계된 API에서는 관찰 가능한 동작이 public API와 일치하고, 모든 구현 세부 사항은 private API 뒤에 숨겨져야 합니다. 

이는 코드 조각이 관찰 가능한 동작이 되려면, 적어도 하나의 클라이언트 목표와 직접적인 관련이 있어야 한다는 것을 의미합니다.

@Image(source: 5-3.png)

시스템의 public API가 관찰 가능한 동작을 넘어 구현 세부 사항을 노출할 때 구현 세부 사항이 유출(leaking) 됩니다.

@Image(source: 5-4.png)
...
캡슐화는 불변 조건 위반(invariant violation)과 같은 코드의 불일치(inconsistencies)로부터 코드를 보호하는 행위입니다.

구현 세부 사항을 노출하는 것은 종종 불변 조건 위반으로 이어집니다. 

초기 User 클래스 예시에서 NormalizeName()을 public으로 두면 클라이언트가 이름 정규화 없이 Name을 직접 변경할 수 있어 불일치가 발생할 수 있었습니다.

캡슐화는 코드 복잡성을 관리하는 데 필수적입니다. 

적절한 캡슐화는 오류 발생 가능성을 제거하여 소프트웨어 프로젝트의 지속 가능한 성장을 가능하게 합니다.

데이터와 작업을 함께 묶는 것(tell-don’t-ask 원칙)은 클래스의 불변 조건이 위반되지 않도록 보장하는 데 도움이 됩니다.

> tell-don’t-ask 원칙: 객체에게 데이터를 요구(Ask) 하지 말고, 객체에게 일을 시켜라(Tell)
...
@Image(source: 5-7.png)

애플리케이션 서비스의 테스트는 전반적인 비즈니스 유즈케이스가 어떻게 실행되는지 확인합니다. 

도메인 클래스로 작업하는 테스트는 유즈케이스 달성에 대한 중간 하위 목적을 확인합니다.

##### 5.3.2 시스템 내부 통신 vs. 시스템 외부 통신

* 시스템 내부 통신
  - 애플리케이션 내부 클래스 간의 통신입니다. 이는 구현 세부 사항입니다. 
  - 도메인 클래스 간의 협업은 클라이언트의 목표와 직접적인 연결이 없기 때문에 관찰 가능한 동작의 일부가 아닙니다.

@Image(source: 5-8.png)
...
@Image(source: 5-9.png)
  
목은 시스템 외부 통신을 검증할 때 유용합니다. 이는 리팩토링 후에도 통신 패턴이 유지되도록 보장하는 데 도움이 됩니다.

반대로 목을 시스템 내부 통신을 검증하는 데 사용하면, 구현 세부 사항에 결합되어 테스트 취약성을 야기합니다.

##### 5.3.3 시스템 내부 통신 vs. 시스템 외부 통신: 예시
...
* 시스템 외부 통신 (관찰 가능한 동작)
  ...
  - EmailGateway.SendReceipt() 호출은 외부 세계에 보이는 부수 효과이며, 클라이언트의 목표(구매 확인 이메일 수신)와 직접 연결됩니다.
...
### 5.4 유닛 테스트의 고전적 학파와 런던 학파 재검토

* 런던 학파의 문제점
  - 런던 학파는 불변(immutable) 의존성을 제외한 모든 의존성에 목을 사용하도록 권장합니다. 
  - 이로 인해 시스템 내부 통신과 외부 통신을 구분하지 않고 목을 사용하게 되며, 결과적으로 테스트가 구현 세부 사항에 결합되어 리팩토링 내성을 상실하게 됩니다. 
...
* 고전적 학파의 강점과 한계
  ...
  - 이는 대부분 SMTP 서비스나 메시지 버스와 같은 시스템 외부 의존성을 의미합니다.
  - 런던 학파보다 낫지만, 여전히 일부 시스템 외부 의존성에 대한 과도한 목 사용을 권장할 수 있습니다.

##### 5.4.1 모든 시스템 외부 의존성을 목으로 대체해야 하는 것은 아니다.

* 의존성 유형 복습
  - 공유 의존성 (Shared dependency): 테스트 간에 공유되어 테스트 결과에 서로 영향을 줄 수 있는 의존성 (예: static mutable field, 데이터베이스).
  - 외부 의존성 (Out-of-process dependency): 애플리케이션 실행 프로세스 외부에서 실행되는 의존성 (예: 데이터베이스, 메시지 버스, SMTP 서비스).
  - 비공개 의존성(Private dependency): 공유되지 않는 의존성.

고전적 접근법은 공유 의존성을 피할 것을 권장합니다. 

테스트 간 실행 컨텍스트 간섭을 유발하여 테스트의 병렬 실행을 방해하기 때문입니다. 
...
* 공유 의존성이 프로세스 외부인 경우 
  - 일반적인 접근 방식은 의존성을 테스트 더블로 대체하는 것입니다.
  - 테스트 실행마다 새 데이터베이스를 생성하거나 새 메시지 버스를 프로비저닝하면 테스트 스위트 속도가 크게 느려지기 때문입니다.

* 관리되는 의존성(managed dependencies)
  - 외부 프로세스 의존성이 오직 해당 애플리케이션을 통해서만 접근 가능한 경우, 시스템의 관측 가능한 행동 범위에 포함되지 않습니다. 
...
목은 흔히 행동을 검증한다고 알려져 있지만 대부분의 경우 그렇지 않습니다.

어떤 클래스가 특정 목표를 달성하기 위해 주변 클래스와 상호작용하는 방식은 관찰 가능한 행동과 무관하며, 이는 구현 세부사항에 불과합니다.

중요한 것은 클라이언트 목표까지 추적 가능한 행동입니다.

목이 행동과 관련이 있는 경우는 오직 애플리케이션 경계를 가로지르는 상호작용을 검증할 때이며, 그 상호작용의 부수 효과가 외부 세계에 가시적으로 드러날 때뿐입니다.
